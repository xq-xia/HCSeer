...
"""
    ...
    1 indicates that the mutation is in the hot spot area, and -1 indicates
    that the mutation is in the cold spot area.
"""
...
print('NOTICE: Annotating by annovar result write to ' + here +
      'annovar/result/Auto-annotation-annovar.exonic_variant_function.txt')
...
# head
file.write(f"{'\t'.join(['chr', 'start', 'end', 'alt',
           'ref', 'otherInfo', 'hot_cold_score'])}\n")
...
for line in input_file:
    # the initialization score is 0, which means that the variant is
    # initialized as neither a cold spot nor a hot spot
    score = 0
    data = line.split('\t')
