...
df = pd.read_csv(
    here +
    'database/tmp/hot_cold_spot-rario-0.90-variant-0.5.txt',
    sep='\t',
    low_memory=False)
...
# Analyze genes that are intolerant to mutations and genes that are
# tolerant to mutations

# get all variants
all_variant = pd.read_csv(
    here +
    'database/tmp/final_variant.txt',
    sep='\t',
    low_memory=False)
...
for gene in only_hot:
    ...
    if len(np.array(get_BLB)) / \
            (len(np.array(get_BLB)) + len(np.array(get_PLP))) <= 0.1:
        analyze_hot += 1
