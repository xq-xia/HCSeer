...
class KDE:
    ...
    def compute_bandwidth(self, x):
        ...
        for i in self.data:
            t = (x - i) / self.hopt
            m += (np.power(t, 2) * self.gaussian_kernel(t)) / \
                (self.n * self.hopt)
            # print(t)
            # print( self.gaussian_kernel(t))
        # print(m - self.density_function(x))
        g = np.power(np.abs(m - self.density_function(x)), 2 / 5)
        h = np.power(self.hopt, 9 / 5) * np.power(c, 1 / 5) * \
            np.power(self.density_function(x), 1 / 5) / g
        return h
...
#     x = np.linspace(-5, 10, 100)
    ...
    fold_kde = KernelDensity(bandwidth=grid.best_params_['bandwidth']).fit(
    ...
    line1, = plt.plot(x, density_estimation, color='r', marker='.',
                      linestyle='-', markersize=2, alpha=0.5, linewidth=3)
    line2, = plt.plot(x, np.exp(log_dens), color='g', marker='.',
                      linestyle='-', markersize=2, alpha=0.5, linewidth=3)
    line3, = plt.plot(x, y, color='b', marker='.', linestyle='-',
                      markersize=2, alpha=0.5, linewidth=3)
