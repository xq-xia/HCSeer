...
grep -P "CLNSIG=Pathogenic;|CLNSIG=Likely_pathogenic;|CLNSIG=Pathogenic/Likely_pathogenic;" $here/database/clinvar_$batch.vcf |grep -E "CLNREVSTAT=criteria_provided,_multiple_submitters|reviewed_by_expert_panel|CLNREVSTAT=practice_guideline" |grep "missense_variant" | awk -F"\t" '{print "chr" $1 "\t" $2 "\t" $3 "\t" $4 "\t" $5 "\t" $8 }' | awk -F"\t" '{split($6,a,";");for(j in a){ split(a[j],m,"=");if(m[1] == "GENEINFO"){split(m[2],g,":");print $1 "\t" $2 "\t" $3 "\t" $4 "\t" $5 "\t" g[1];}}}' > $here/database/update_database/clinvar-$batch-PLP-temp.vcf

grep -P "CLNSIG=Likely_benign;|CLNSIG=Benign;|CLNSIG=Benign/Likely_benign;" $here/database/clinvar_$batch.vcf | grep -E "CLNREVSTAT=criteria_provided,_multiple_submitters|reviewed_by_expert_panel|CLNREVSTAT=practice_guideline" |grep "missense_variant" | awk -F"\t" '{print "chr" $1 "\t" $2 "\t" $3 "\t" $4 "\t" $5 "\t" $8 }' | awk -F"\t" '{split($6,a,";");for(j in a){ split(a[j],m,"=");if(m[1] == "GENEINFO"){split(m[2],g,":");print $1 "\t" $2 "\t" $3 "\t" $4 "\t" $5 "\t" g[1];}}}' > $here/database/update_database/clinvar-$batch-BLB-temp.vcf

grep -P "CLNSIG=Uncertain_significance;" $here/database/clinvar_$batch.vcf | grep -E "CLNREVSTAT=criteria_provided,_multiple_submitters|reviewed_by_expert_panel|CLNREVSTAT=practice_guideline" |grep "missense_variant" | awk -F"\t" '{print "chr" $1 "\t" $2 "\t" $3 "\t" $4 "\t" $5 "\t" $8 }' | awk -F"\t" '{split($6,a,";");for(j in a){ split(a[j],m,"=");if(m[1] == "GENEINFO"){split(m[2],g,":");print $1 "\t" $2 "\t" $3 "\t" $4 "\t" $5 "\t" g[1];}}}' > $here/database/update_database/clinvar-$batch-VUS-temp.vcf

grep -P "CLNSIG=Conflicting_classifications_of_pathogenicity;" $here/database/clinvar_$batch.vcf | grep -E "CLNREVSTAT=criteria_provided,_multiple_submitters|reviewed_by_expert_panel|CLNREVSTAT=practice_guideline" |grep "missense_variant" | awk -F"\t" '{print "chr" $1 "\t" $2 "\t" $3 "\t" $4 "\t" $5 "\t" $8 }' | awk -F"\t" '{split($6,a,";");for(j in a){ split(a[j],m,"=");if(m[1] == "GENEINFO"){split(m[2],g,":");print $1 "\t" $2 "\t" $3 "\t" $4 "\t" $5 "\t" g[1];}}}' > $here/database/update_database/clinvar-$batch-CON-temp.vcf
