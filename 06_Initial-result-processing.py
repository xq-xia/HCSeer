...
'''
input file :outputfile of EM.py
output file:final result of hotspot area and coldspot area
...
def initial_result_processing():
    ...
    result.write(
        'type' + '\t' + 'chr' + '\t' + 'gene' + '\t' + 
        'iso' + '\t' + 'aa_start_pos' + '\t' + 
        'aa_end_pos' + '\t' + 'PLP_number' + 
        '\t' + 'BLB_number' + '\t' + 'Ratio_ratio' + '\n')
    ...
    for line in file:
        ...
        if len(PLP) != 0:
            ...
            for i in range(0, len(hotspot) - 1):
                ...
                if content[0] == '0':
                    content[0] = '1'
                # if float(content[4]) >= 0.90 and float(content[2]) / (float(content[1]) - float(content[0])) >= 0.5:
                if float(content[4]) >= 0.90:
                    result.write(
                        'hotspot' + '\t' + chromosome + '\t' + gene + '\t' + iso + '\t' + content[0] + '\t' + content[
        ...
        if len(BLB) != 0:
            ...
            for i in range(0, len(coldspot) - 1):
                ...
                if content[0] == '0':
                    content[0] = '1'
                # if float(content[4]) >= 0.90 and float(content[3]) / (float(content[1]) - float(content[0])) >= 0.5:
                if float(content[4]) >= 0.90:
                    result.write(
                        'coldspot' + '\t' + chromosome + '\t' + gene + '\t' + iso + '\t' + content[0] + '\t' + content[
                            1] + '\t' + content[2] + '\t' + content[3] + '\t' + content[4] + '\n')
